...
- **事实核验前置**：任何级别的任务在规划前都先核对【事实依据】——需求点名的文件/类/表是否真实存在。若需求说"改 X 文件"但项目里没有 X（虚假前提），系统识别后【强制转人工澄清】而非基于假前提硬跑（即使自动化模式也会拦下，不浪费算力产垃圾）。产物经确定性闸门验证通过后回灌知识库，让后续任务的事实核验始终基于最新状态。
