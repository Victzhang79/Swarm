...
[![Version](https://img.shields.io/badge/version-0.9.18-blue.svg)](https://github.com/Victzhang79/Swarm/releases)
...
全程无需你点名改哪个文件、无需你逐行审查 —— 你描述"要什么"，Swarm 负责"怎么对地做出来"。
...
- **集成级不假绿**：L2 对全工程做真编译（多模块 reactor）；工具链缺失时**拒绝放行而非静默跳过**，绝不把"没验证"当"验证通过"。
...
- **系统性 fail-closed 加固**：默认拒绝（安全/正确性状态缺省判否）、数据先写后删（重建索引写新代际成功再删旧）、临时验证回滚**仅限改动涉及文件**（绝不整库 `clean -fd` 抹用户改动）、读路径补 workspace 边界复校、跨项目资源按归属鉴权。
...
- **每个修复批次经双重对抗复核**（独立 code-reviewer + silent-failure-hunter 逐条核实），复核抓到的问题当批整改；关键判定（如"某疑似 bug 实为 fail-safe"）须三方证据链一致才定案。
- **编排图封闭性有静态审计**：所有路由函数返回值 × 分支表 × 节点存在性交叉验证，无死端、无不可达节点、无未覆盖分支——状态机不会把任务走进死胡同。
...
| 层 | 机制 |
|---|---|
| **认证/授权** | 多用户 Token + RBAC（全局角色 + 项目级成员权限）；首次登录强制改密；`SWARM_TOKEN_TTL_HOURS` 限令牌暴露窗口 |
| **浏览器会话** | SSE/WebSocket 走 **HttpOnly Cookie**（token 不进 URL/日志/Referer）；失权即断流，不静默重连 |
...
| **执行隔离** | CubeSandbox 容器：非 root、网络封锁、资源限额；主机与目标项目工作区经路径边界校验，防穿越 |
...
| **注入面治理** | 所有 shell 拼接经 `shlex.quote`（全仓扫描纪律）；上传/摄取路径有 SSRF/穿越校验；LFI 信任边界设在任务入口 |
...
| `GET /api/health/ready` | 就绪探针（容器 HEALTHCHECK/编排门） | fail-closed 真探活：PG 必查、Redis 按启用判、Qdrant 含本地模式兜底；任一启用依赖不可达 → 503 + 明细 |
