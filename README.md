...
> 基于 LangGraph 的多智能体协作编程系统 —— 让 AI 像团队一样拆解、编码、验证、学习。

Swarm 把一个编程需求交给一套协作的 AI 智能体：**Brain** 负责理解需求、做技术设计、拆解子任务；**Worker** 在隔离沙箱中实际写代码、跑构建、自我验证；**知识库**为每个任务注入项目上下文；**记忆系统**让系统从每次审核反馈中持续学习。整套流程通过 Web UI 或 CLI 驱动，支持云端与本地模型混合编排。
...
- **代码知识库**：基于符号表 + 向量检索（embedding + rerank，可配云端或自建），为每个任务精准注入相关代码上下文。
- **确定性验证闸门**：Worker 产物先过确定性 L1 闸门（编译/测试/lint），再走 LLM 审查，最后人工 accept。
- **记忆学习闭环**：每次审核反馈沉淀为分层记忆（L0–L6），影响后续编排与生成。
- **配置式、开箱即用**：模型、沙箱、Embedding/Rerank 接入点均可在 Web UI 配置，敏感 Key 加密存储，保存即生效。
