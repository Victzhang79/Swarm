...
[![Tests](https://img.shields.io/badge/tests-8700%2B%20passing-brightgreen.svg)](#-系统自身如何被验证)
...
flowchart LR
    ...
    subgraph EXP[“🧩 策展经验库”]
      direction TB
      SK[48+ 内置技能<br/>栈×意图×阶段×子任务 精确路由]
...
- **8700+ 行为测试**在全新空 PostgreSQL + Python 3.12 的 CI 上全量跑，每个 commit 必绿才合入。测试写**行为断言**而非结构断言——不焊死实现，重构不脆；修 bug 先写红的复现测试（test-first），语义演进时旧测试按拍板显式更新并标注机制编号，绝不悄悄迁就。
...
CI 在全新空 PostgreSQL（pgvector）+ Python 3.12 环境下运行 lint 与全量测试（当前 **8700+ passed**）；
