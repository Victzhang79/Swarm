...
[![Version](https://img.shields.io/badge/version-0.9.28-blue.svg)](https://github.com/Victzhang79/Swarm/releases)
...
- **合并 fail-closed + 验收有牙**（v0.9.28）：并行子任务合并的每条"静默丢码"路径全部封死——新文件多写者不一致进 rebase 重生成（带保留方最新内容防同形重试）、rebase 超限按来源分流（真源码被丢→升级人工，绝不静默交付）、3-way 基线漂移即拒绝（不产出语义损坏的"干净"合并）、硬冲突标记绝不混入交付 diff；契约检查从"全缺才报"升级为缺失率阈值+逐符号词边界归因 owner 定向重派；鉴权接口的验收断言可真执行（冒烟内登录取 token，登录基础设施失败如实判 inconclusive 不冤枉代码）。
- **中止与恢复统一协议**（v0.9.28）：重试不再从零重跑——新 thread 播种上一轮已通过产物与覆盖水位；任务状态终态 CAS 防"晚到写复活已取消任务"；独立看门狗守护墙钟/锁续期（节点内悬挂不再零保护）；墙钟/失锁与 token 超限同走 PARTIAL 抢救绝不裸 FAILED 丢产物；PG checkpointer 连接池自愈；周期孤儿对账；模块锁按**全部写集**组合互斥（写集重叠必互斥、不相交照常并行，升级冲突有界排队绝不纸面互斥照跑）。
- **预算脊柱 TaskLedger**（v0.9.28）：全任务 token 预算单点预留-结算（router 单点闸，error 路径也入账）、账本落库跨重启延续、各重试层统一从账本扣减——饱和 provider 下确定性 escalate，费用可审计。
- **配置面冻结**（v0.9.28）：206 个 SWARM_* 开关全量登记（新增不登记=测试红）+ dev/e2e/prod 三 profile 推荐组合；终态报告带机读 `degraded_summary`（按机制聚合），一眼判读该轮降级了什么。
...
### 🧩 经验拔插层：把"成功经验"按上下文喂到模型跟前（v0.9.27，v0.9.28 混合架构升级）
...
- **worker 侧 push+pull 混合**（v0.9.28）：最相关的一条**栈/框架特化**经验全文直接进提示词（零迭代成本），其余按需挂成离散工具 `experience__<id>`（≤3 个，按"触发条件"描述让小模型精准判别何时调用）；选择器带框架级相关性（FastAPI 项目绝不推 Django 建议）与 DB 依赖探测互斥挂载（探出 MySQL 才挂 MySQL 经验）。planner 侧直接全文注入。每次经验调用留结构化遥测（skill×子任务），加分/减分数据可证伪。
- **系统级编写 / 导入（跨项目）**：WebUI 管理员「🧩 技能」页可**结构化编写**或**粘贴 `.md` 导入**（支持第三方 `SKILL.md` 导入，须含 `description` 触发条件——无判别依据的宽默认候选不放行），落库对所有项目生效。
- **严格准入闸**：每次入库都过校验——schema / 密钥扫描 / 提示注入与危险指令拦截 / **标题↔正文意图一致性**（LLM 裁判专抓"标题说读、正文却写"这类不一致），不合格即拒绝，绝不让乱七八糟或意图不明的技能进库。v0.9.28 补**挂载预览**（保存前展示会出现在哪些 栈×意图 的工具面及排位）、高 priority 二次确认、克隆同 id 覆盖内置（下架件复活需显式确认）。
