...
### 批 1：checkpointer 接通 PG（地基：CoordinationBackend 雏形 + checkpointer 持久化）
1. [ ] `get_compiled_brain_graph()` 改为优先走 PG checkpointer（接通已有的 `compile_brain_graph_with_postgres`）。
2. [ ] **async 生命周期处理**：AsyncPostgresSaver 在 FastAPI lifespan 内初始化（⚠️ 记忆中的坑：async 单例绑 ephemeral event loop 会 hang——绝不能在 asyncio.run-per-call 处建；用 lifespan + 长生命周期连接）。
3. [ ] checkpointer 表迁移：langgraph 自带 `.setup()`，并入 scripts/init_db.py 统一入口（与现有 schema 单一事实源一致）。
4. [ ] **降级保障**：PG 不可用 → 回退 MemorySaver + warning（保 CI/本机开箱即用，RBAC-off 314 测试不破）。
5. [ ] 验证：起 2 个进程共享同 PG，进程 A 发起 interrupt 任务，进程 B `Command(resume=)` 能恢复同一 thread。

### 批 2：调度器选主（地基：SchedulerLeadership 抽象）
1. [ ] 定义 `CoordinationBackend` 接口 + `PgCoordinationBackend`（`pg_try_advisory_lock` 封装）。
2. [ ] `SchedulerLeadership(backend, lock_key)`：`is_leader()` / 自动续租 / 释放。
3. [ ] 4 个调度器（task/kb_update/consistency/memory_decay）包一层"仅 leader 执行"；非 leader 待命（不退出，主挂了能接管）。
4. [ ] **降级保障**：backend 不可用 → 退化为"本进程即 leader"（单机行为不变）。
5. [ ] 验证：2 进程只有 1 个执行 consistency；kill leader 后另一个在租约过期内接管。

### 批 3：沙箱池实例隔离（地基：SandboxPoolStrategy 抽象 + 复用本轮 metadata）
1. [ ] 每实例生成 `swarm_instance=<uuid>`（进程级稳定 ID）。
2. [ ] `manager.create` 时打 `metadata={"swarm_instance": <id>}`（复用 v0.6.x 实测的 CubeSandbox metadata 写入能力）。
3. [ ] `_sweep_startup_orphans` 改为按 `SandboxQuery(metadata={"swarm_instance": <本实例>})` 过滤，只 kill 本实例残留——**替换 v0.6.2 的 opt-in 开关止血方案**（开关退役）。
4. [ ] `SandboxPoolStrategy` 接口预留 `SharedPool` 扩展点（不实现）。
5. [ ] **降级保障**：metadata 不可用 → 回退当前 opt-in 开关行为。
6. [ ] 验证：2 实例各创建沙箱，实例 A 重启清扫不误杀实例 B 的沙箱（用实测探针验证）。
