...
## T7 调查结论（2026-07-16，三路取证：worker loop / round56 接线 / round63 日志）

**取证实锤（logs_archive/round63_postmortem/swarm.noheartbeat.log）**：3 个确认复读 worker——
st-2-1-1（L3926，截断类名 `IllegalArgumentExce` 复读并写入源码→下游 cannot find symbol）、
st-2-1-1-2（L7464，`IllegalArgumentEx` 洪泛，4 次重启 3 次撞满 900s≈2700s 白烧）、
st-8（L6429，整句循环「我意识到我一直在犯一个循环错误」+`LinkedHash Map` 自相矛盾，迭代上限
95+900s 双终止）。肇事模型全部 Qwopus3.6-27B-v2-NVFP4（4-bit 量化）；**复读载体是正文
content 非 thinking**；单 turn 内 token 级复读 + 跨重启复进同一循环两层并存。

**机制真相**：①worker LLM 恒流式，唯一 chunk 经手点=`router._astream_inner`；全仓零 token 级
复读检测（既有全是错误签名/失败指纹级）。②round56「思考失控先换模型」：换挡动作 brain/worker
双侧已接（`swarm_no_fallback` 链尾标记 router.py:1024-1029），但**判据纯墙钟+content_seen
布尔**——本地 worker 关 thinking 且预算对 local 取 0 → 永不触发；复读时 chunk 持续产出，
stall/墙钟/max_tokens 全为盲区。③升档通路现成：`l1_decision_source→FINDING-12 force_strong→
_select_pool_override→routing_complex`。④**T7-0 隐性真 bug（调查中实锤）**：ChatGenerationChunk
无 `.content` 属性 → router.py 旧 `getattr(chunk,'content')` 恒空 → `content_seen` 永假 →
R55「正文已开吐绝不重开」保护对真实流失效；旧 test_r55 用自带 .content 的假 chunk 假绿
（测试建模失真=假绿的又一实例）。

### T7 完成（2026-07-16，test-first + 对抗双复核 + 全量套件）
**治本**：①`models/degeneration.py`（新）StreamRepetitionDetector 滑窗双通道（word_flood 词洪泛
=count+覆盖率+词多样性+上下文模板比；segment_loop 句循环=同句 ≥4×+合计覆盖率），外加**句多样性
总闸**（新句占比>0.25 且句数≥6 → 两通道都不判）；钩在 `_astream_inner` 每 chunk，触发即
aclose+WARNING+抛 `StreamDegenerationError`。②换挡：classify_failure isinstance 优先归
CAPABILITY；非链尾 with_fallbacks（ainvoke 全调用语义，流中途照样接）同请求切下一模型；链尾冒泡
executor 异常路径 `exception_l1_details`→`degeneration_hard_fail`→FINDING-12 force_strong 升
最强模型。③T7-0 修正：`_gen_chunk_text` 权威取 .text（TextAccessor 不走方法式调用）、list
content block 拼接；R55 测试升级真 chunk 形态（升级即红=bug 实锤）。④配置 4 键
SWARM_MODEL_STREAM_DEGEN_*（enabled 默认开；覆盖率/多样性成对阈值固化模块常量防只调一半）。
**register 口径修正（入册）**：原文「≥3× 立即中止」不采纳字面——round63 语料 `public`×2995，
纯计数在真实代码必误杀；改密度+多样性判据，由合法样本（import 块/常量/pom/stub/规划 JSON/
PRD/共享中文 Javadoc/SQL 种子）与实锤样本双向校准锁死。
**对抗双复核 6 条全治全锁**：猎手 F1(CRITICAL)=`_run_agent`「撞迭代上限」子串启发式会吞
needle 含 recursion 的退化异常→isinstance 短路上抛+对照锁；F2=探测器自身异常杀健康流→
fail-open 停用本流探测+WARNING；F3=TextAccessor callable 恒真→每 chunk 弃用调用（实测 165 条）
→`callable and not isinstance(str)`；F4=list content 判空复活老 bug→block 拼接；复核
R-F1(HIGH)=segment 通道对中文合法高重复系统性误杀（规划 JSON 共享验收句 ×5 在 648 字符处被杀
实锤）→句多样性总闸；R-F2=SQL 种子行内双列同值恰打 ctx 0.5 临界→严格小于+总闸双防。
**revert-check**：F1 回退红✓/句多样性闸回退红✓/ctx 严格化被总闸冗余覆盖（双防线，可接受）。
**已知留观**：近似变体循环（每轮措辞微变、低于字面重复阈值）仍靠迭代上限/墙钟兜底（宁缺勿滥）；
brain 云端 reasoning 复读由 reasoning 通道+R55 时间预算双护；langchain 2.0 若移除 TextAccessor
需重审 T7-0（测试有前提断言哨兵）。35 测试（22 主 +10 复核锁 +R55 升级 3+1）。

## T8 调查结论（2026-07-16，两路取证：L1 fix-loop 全景 / round63 空烧量化）

**取证实锤**：`upstream_module_broken` 确定性信号 10:18:21 首现（L5036），但 st-8 三阶段
（CODING/VERIFYING/PRODUCING）各撞 95 迭代、3 轮共烧 ≈41min（L5285/5462/7372，第 3 轮
PRODUCING 622.8s 发生在 blocked 判定之后）；brain 三次 retry 全投未清洗环境（L5755/6652/
7603）。越界方是 auto-repair 非 agent（`repaired_file_paths=['./pom.xml','./ruoyi-framework/
pom.xml']`，scope_violations 全空）——污染源=T1 已治、repair scope 守卫=T2 已治、brain 误判
transient/基线死锁=T3 已治；T8 剩下的是 **worker 侧四个 fail-fast 缺口**：
①`_build_error_is_upstream` 用 diff/-pl 近似 scope 归属（空 diff/无 -pl 双失灵→build_failed
硬 FAIL 烧修复轮）；②依赖解析级错误（Could not find artifact 内部坐标）零文件路径→errs_files
恒空盲区（_build_blocked_on_unbuilt_internal 只管 package 级）；③BLOCKED 后 _phase_produce
仍无条件跑 produce LLM 步+Phase-4 复核；④BLOCKED 时 det_ok=None→verify agent 步照跑
（输出恒被仲裁器丢弃=纯浪费）。fix 循环退出条件全景核实：无任何一条看"错误位置是否在
scope 内"。徒劳 repair 循环经 T2 scope 守卫已自然零修复即退，不另设闸（宁缺勿滥）。

### T8 完成（2026-07-16，test-first + 对抗双复核 + 全量套件）
**治本**：①scope 写权确定性通道（FileScope.is_writable 归属判据优先，任一报错文件在写权集
→False 绝不推卸；allow_any 让位旧启发式；异常 fail-open 落旧启发式+WARNING）；②坐标→注册
模块 pom 映射（_maven_modules 反查，第三方坐标空集交防线④）；③_phase_produce BLOCKED
短路（跳过 produce LLM 步/Phase-4 复核/DEBUG failing_test 闸，diff 照常回传不丢工作，上报
文本带 kind+blocked_on_modules）；④_should_run_verify_agent（BLOCKED 跳过 verify agent 步，
always 逃生口保留）。brain 契约键（pipeline_blocked/blocked_on_modules/blocked_on_files/
not_run_kind）字面不变，T3/C9 零回归，且新增 upstream_judge_channel 判据通道留痕。
**对抗双复核 5 条全治全锁**：R-MED=坐标映射不比版本会把引用方幻觉版本（round47 类）诬告
健康兄弟为 upstream→版本一致性闸（不符不映射留 fix 循环源头修；属性形态不可判定保守仍映射）；
猎手 F1=scope 异常静默降级→WARNING；F2=读树失败静默空集→WARNING；F3/R-LOW=DEBUG
failing_test 闸未收口于短路→and not _ff_kind；F4=判据通道不可追溯→evidence_out+details 留痕。
时序粘滞/合成文本解析/空 l1_details/H2 回滚可达性/契约键传递链经猎手逐条核清非 bug。
...
## T10 核实结论（2026-07-16，核实型任务——前提证伪，无需治本）
**「34 worker 零 experience__* 调用」是 R40-3 设计使然，不是断线**：worker_pull_enabled
默认 False（settings.py:695，R40-3 定案注释=两轮 tool-telemetry 实证小模型不接可选离散
工具、pull 调用恒 0 纯占工具槽）→ service.py pull_budget=0 → build_worker_experience_tools
返回 []（被 test_r40_push_shift.py:42 显式锁定）。经验层实际走 **push 通道（top-K 全文注入
worker prompt）**，round63 端到端健康【亲核】：34 条 [skills-telemetry] worker_push 恰对应
34 worker、技能选择正确（写 pom→maven-*、写 Mapper→jpa-patterns，栈轴走依赖坐标+英文
文件路径信号，中文描述不构成零命中风险）、0 条 experience__ 字符串、0 条 [skills] 告警
