...
[![Tests](https://img.shields.io/badge/tests-8700%2B%20passing-brightgreen.svg)](#-how-the-system-itself-is-verified)
[![Version](https://img.shields.io/badge/version-0.9.81-blue.svg)](https://github.com/Victzhang79/Swarm/releases)
...
- **8700+ behavioral tests** run in full on a pristine PostgreSQL + Python 3.12 CI; every commit must be green to merge. Tests assert **behavior, not structure** — nothing is welded to an implementation, so refactors don't shatter them; bug fixes start with a red reproduction (test-first), and when semantics deliberately evolve, old tests are explicitly updated by decision with a mechanism annotation, never quietly bent.
...
CI runs lint and the full suite on a pristine, empty PostgreSQL (pgvector) + Python 3.12 environment (currently **8700+ passed**);
