...
★本册的边界＝**生产**开关，测试期开关刻意不登记（#29-4 T-7 实测后的裁决）★
`test_phase7_env_registry._SCAN_DIRS` 只扫生产包，且第二条测试是**双向**的
（登记了但代码里扫不到 ⇒ 判死条目 ⇒ 红）。所以往册里加一个只被 `test/` 读的开关
**结构上必然红**，不是漏配。
实测过另一条路（把 `test/` 纳入扫描面）：`test/` 出现 206 个唯一 SWARM_* 名，其中
**32 个未登记，绝大多数是故意造的假名**（`SWARM_BAD`、`SWARM_DEFINITELY_NOT_EXIST_X`、
`SWARM_A/B/C/D` 等——它们正是"未登记开关必须被拒"这类测试的夹具）。把它们登记进来
会让本册从"生产配置面的单一事实源"退化成"仓内出现过的字符串集合"，反而丧失意义。
故：测试期开关（如 `SWARM_TEST_REQUIRE_SERVICES`）在**其定义处**（`test/conftest.py`）
就近注释说明，不进本册。
"""
...
REGISTERED_ENVS: dict[str, str] = {
    ...
    # P-H4b：crates.io 依赖版本解析（cargo 脚手架 driver），与 npm/go/pypi 三栈 LOOKUP
    # 开关同义栈中立铺开。0=关闭（单测默认关）；关闭后=解析不到即如实丢弃（含工程清单
    # 与 Cargo.lock 证据层）。
    "SWARM_CARGO_LOOKUP": "brain/cargo_registry.py:_lookup_enabled",
    "SWARM_CARGO_LOOKUP_TIMEOUT_S": "brain/cargo_registry.py:_HTTP_TIMEOUT_S",
    ...
    "SWARM_ALLOW_EXTERNAL_PROJECT_PATH": "api/routers/project.py:_env_allow_external_project_path",
    ...
    "SWARM_AUTO_ACCEPT": "api/routers/task.py:_resolve_create_auto_accept",
    ...
    "SWARM_QUOTE_COVER_MIN": "brain/requirements_grounding.py:_quote_grounding_params",
    ...
    "SWARM_QUOTE_MIN_TILE_CHARS": "brain/requirements_grounding.py:_quote_grounding_params",
