...
- **T1 ✅ 698b644**：证据强度分层四类分类器（对抗双复核 2H+2M 全 CONFIRMED 全整改；
  17 用例含真 fixture 回归；revert-check 12/17 红；全量 4600 项 0 失败）。
- **T3 ✅ 01acd67**：外科让路（同泄压阀+try 兜底）+ 同签名熔断（SWARM_G1_RETRY_FUSE 泄压阀
  + retry 绑定签名 + 三 replan 出口/validate 通过/人工 REVISE 全对称清理——复核逮出的
  REVISE 软锁死/跨周期相邻巧合误熔断全治）。
- **T4 治毕待复核**：①契约 per-module prompt 注入该模块 file_plan 文件名+命名铁律
  （_contract_module_files_block，源头预防：round64 实锤契约 prompt has_file_plan=False，
  两命名空间独立产生漂移必然）②pin 零命中分支 INFO 留痕（round65 漂移残量观察面）。
  调查报告定案：R63-T4 pin 是 name→basename JOIN，结构上治不了"名字对不上"；确定性模糊
  改写无高精度判据（difflib 坑）否决；契约与 file_plan 共享命名权威登记为长期方向。
- **T5 ✅ 治毕待复核**：诚实改判——seq6 不是复读退化，是【思考失控】（28841 reasoning
  chunk 零正文）且 R56-1 无损切备闸（600s）被节点超时（500s）抢跑、结构性够不着。
  治本=_stage2_module_timeout()=max(500, 思考预算+120) 确定性排序。
- **T6① 治毕待复核**：录制器 msg 分支补采 generation_info（finish_reason 真实落点，
  round64 全 58 行皆 None 的病根）。
- **T7② 结案（非问题）**：model_swap_audit 判无残留无漂移；启动告警=API 拉起瞬间探测
  抖动（soak sustained 全绿实证），运行期熔断半开自愈已覆盖（round45 拍板：掉线≠退役）。
- **T7① 结案（诚实划界·B类）**：ledger 结算依赖 langchain 回调，取消类中止（BaseException）
  可能不触发回调；但 C7 inflight 登记+TTL 兜底双保险存在且工作正常（48fa3b51 被 TTL 按估算
  结算，账未失明）。单发、自愈、有 WARNING 观察面 → 不专项治，round65 盯该 WARNING 复现率。
- **T7③ 治毕待复核**：_failed_machine_account 增 plan_validation 诊断键
  （retry_count+last_issues），FAILED@PLAN 免日志考古。
