    ...
    async with session.request(
        ...
        for key, value in upstream.headers.items():
            ...
            if lowered in HOP_BY_HOP_HEADERS or lowered == "content-length":
                continue
            # CubeProxy 偶发误标 Content-Encoding:gzip，转发后 httpx 解压失败
...
async def proxy_sandbox(request: web.Request) -> web.StreamResponse:
    ...
    # 每次请求读取最新 proxy 配置（避免 app 启动时 env 尚未注入）
    cube_proxy_base = _strip_slash(_env("CUBE_REMOTE_PROXY_BASE", config["cube_proxy_base"]))
    sandbox_domain = _env("CUBE_REMOTE_SANDBOX_DOMAIN", config["sandbox_domain"])
    url = _join_url(cube_proxy_base, upstream_path, request.query_string)
