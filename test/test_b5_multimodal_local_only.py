...
def _router(kind_of):
    ...
    def _prov(m):
        p = MagicMock(); p.kind = kind_of(m); p.id = p.kind; return p
...
def test_prefers_local_even_when_cloud_has_bigger_context():
    ...
    rows = [
        {"provider_id": "cloud", "model_id": "cloud/vl", "supports_multimodal": True, "source": "probed", "context_window": 200000},
        {"provider_id": "local", "model_id": "local-vl", "supports_multimodal": True, "source": "probed", "context_window": 8000},
    ]
...
def test_none_when_only_cloud_vl_available():
    ...
    rows = [{"provider_id": "cloud", "model_id": "cloud/vl", "supports_multimodal": True, "source": "probed", "context_window": 200000}]
    with patch("swarm.models.capability_store.list_capabilities", return_value=rows):
        got = router._multimodal_model_from_capabilities()
...
def test_local_vl_autodiscovered_without_in_use_list():
    ...
    rows = [{"provider_id": "local", "model_id": "local-vl", "supports_multimodal": True, "source": "probed", "context_window": 8000}]
    with patch("swarm.models.capability_store.list_capabilities", return_value=rows):
        got = router._multimodal_model_from_capabilities()
    assert got == "local-vl", got
...
def _route_router(configured_mm: str):
    ...
    cfg.routing_multimodal_fallback = ["LOCAL_PRIMARY_MODEL"]
...
def test_configured_multimodal_wins_over_stale_downlined_row():
    ...
    router = _route_router("Qwen3.8-27B-NVFP4")
...
def test_autodiscovery_kicks_in_when_configured_not_multimodal():
    ...
    rows = [{"provider_id": "local", "model_id": "local-vl", "supports_multimodal": True,
