...
def test_normalize_coerces_dedupes_and_drops_garbage():
    ...
    assert out == [
        {"id": REQ_A, "reason": "现有代码已实现", "evidence": "", "evidence_files": []},
        {"id": REQ_B, "reason": "", "evidence": "", "evidence_files": []},
    ]
...
def test_matrix_baseline_declaration_counts_as_covered():
    ...
    assert m["baseline_covered"] == [{"id": REQ_B, "reason": "存量已有", "evidence": "", "evidence_files": []}]
...
async def test_plan_writes_baseline_covered_state_key(monkeypatch):
    ...
    assert out["baseline_covered"] == [{"id": REQ_B, "reason": "现有代码已满足", "evidence": "", "evidence_files": []}]
...
async def test_plan_batched_unions_baseline_declarations():
    ...
    assert failed == []
    assert baseline == [{"id": REQ_B, "reason": "存量模块已有", "evidence": "", "evidence_files": []}]
...
def test_normalize_keeps_best_reason_on_duplicate():
    ...
    assert out == [{"id": REQ_A, "reason": "后到但有依据", "evidence": "", "evidence_files": []}]
