...
def test_rollback_on_reload_gate_failure(tmp_path, monkeypatch):
    ...
    with pytest.raises(HTTPException) as ei:
        cfgmod._persist_env_updates({"SWARM_TESTKEY": "new"}, is_admin=True, who="test")
...
def test_rollback_removes_newly_added_key(tmp_path, monkeypatch):
    ...
    with pytest.raises(HTTPException):
        cfgmod._persist_env_updates({"SWARM_BRANDNEW": "x"}, is_admin=True, who="test")
...
def test_commit_on_success(tmp_path, monkeypatch):
    ...
    cfgmod._persist_env_updates({"SWARM_OKKEY": "v1"}, is_admin=True, who="test")
