...
# 模型路由（worker 全本地小模型；*_FALLBACK 为多级兜底链，逗号分隔，主→次→兜底）
# 本地模型名必须与服务器实际列表完全一致
