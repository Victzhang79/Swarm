...
SWARM_MODEL_BRAIN_PRIMARY=REMOTE_BRAIN_PRIMARY
SWARM_MODEL_BRAIN_FALLBACK=REMOTE_BRAIN_FALLBACK

# 模型 — 本地
SWARM_MODEL_LOCAL_BASE_URL=${LOCAL_URL}
SWARM_MODEL_LOCAL_API_KEY=${LOCAL_KEY}
SWARM_MODEL_WORKER_PRIMARY=LOCAL_PRIMARY_MODEL
...
SWARM_MODEL_WORKER_LOCAL=LOCAL_OLLAMA_MODEL

# 模型路由（worker 全本地小模型；*_FALLBACK 为多级兜底链，逗号分隔，主→次→兜底）
# 本地模型名必须与服务器实际列表完全一致
SWARM_MODEL_ROUTING_TRIVIAL=LOCAL_SMALL_MODEL
SWARM_MODEL_ROUTING_TRIVIAL_FALLBACK=REMOTE_FAST_MODEL,LOCAL_NVFP4_MODEL
SWARM_MODEL_ROUTING_MEDIUM=REMOTE_FAST_MODEL
SWARM_MODEL_ROUTING_MEDIUM_FALLBACK=LOCAL_LARGE_MODEL,LOCAL_NVFP4_MODEL
SWARM_MODEL_ROUTING_COMPLEX=LOCAL_PRIMARY_MODEL
SWARM_MODEL_WORKER_FALLBACK=LOCAL_LARGE_MODEL
SWARM_MODEL_ROUTING_COMPLEX_FALLBACK=REMOTE_FAST_MODEL,LOCAL_NVFP4_MODEL,LOCAL_LARGE_MODEL
SWARM_MODEL_ROUTING_MULTIMODAL=LOCAL_NVFP4_MODEL
SWARM_MODEL_ROUTING_MULTIMODAL_FALLBACK=LOCAL_PRIMARY_MODEL
