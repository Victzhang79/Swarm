...
# D8①（阶段6，登记册 §五）：新增 "bearer"——运维配置冒烟登录（SWARM_SMOKE_LOGIN_*）
# 时鉴权断言可执行（登录取 token 注入 Authorization），未配置时 bearer 照旧降 manual
...
def validate_assertions(
    ...
    for item in raw:
        ...
        if spec_id in seen_ids:
            ...
            continue
        # D8②（阶段6，登记册 §五）：不再按到达序硬截 30 条——晚到 req 的断言整批被丢
        # =验收面系统性偏斜。改收尾按 req 分桶配额选取（见循环后 _bucket_select）。
    ...
    # D8②：按 req 分桶配额选取——每个 req 轮转保底（round-robin），直到全局帽
    # MAX_ASSERTIONS。到达序硬截会把晚到 req 的断言整批丢弃（验收面偏斜）；分桶保证
    # 每个被断言覆盖的 req 至少留 1 条（req 数 > 帽时也按轮转公平截断+留痕）。
    if len(valid) > MAX_ASSERTIONS:
        _by_req: dict[str, list[dict]] = {}
        _req_order: list[str] = []
        for it in valid:
            _rid = str(it.get("req_id") or "")
            if _rid not in _by_req:
                _by_req[_rid] = []
                _req_order.append(_rid)
            _by_req[_rid].append(it)
        _selected: list[dict] = []
        _round = 0
        while len(_selected) < MAX_ASSERTIONS and any(_by_req.values()):
            for _rid in _req_order:
                if _by_req[_rid]:
                    _selected.append(_by_req[_rid].pop(0))
                    if len(_selected) >= MAX_ASSERTIONS:
                        break
            _round += 1
...
def assertion_to_probe_cmd(spec: dict[str, Any], port: int | str) -> str:
    ...
    if _bearer:
        # D8①：token 由登录段 export（smoke_login_cmd）；空 token=裸打（401 如实计败）
        curl_parts.append('-H "Authorization: Bearer ${SWARM_SMOKE_TOKEN}"')
    if body_json is not None:
...
def smoke_login_cmd() -> str | None:
    ...
    path = (_os.environ.get("SWARM_SMOKE_LOGIN_PATH") or "").strip()
    body = (_os.environ.get("SWARM_SMOKE_LOGIN_BODY_JSON") or "").strip()
    if not path or not body or not path.startswith("/"):
        return None
    try:
        _json.loads(body)
    except Exception:  # noqa: BLE001 — 配置非法=不启用（loud）
        logger.warning("[ACCEPT] SWARM_SMOKE_LOGIN_BODY_JSON 非法 JSON，登录段不启用")
        return None
    field = (_os.environ.get("SWARM_SMOKE_TOKEN_FIELD") or "token").strip()
    _extract = (
        "import sys,json,functools;d=json.load(sys.stdin);"
        f"print(functools.reduce(lambda o,k:(o or {{}}).get(k) if isinstance(o,dict) else None,"
        f"{field.split('.')!r},d) or '')"
    )
    return (
        'SWARM_SMOKE_TOKEN="$(curl -sS -m 15 -X POST '
        f'-H "Content-Type: application/json" --data {_shlex.quote(body)} '
        f'"http://127.0.0.1:${{SMOKE_PORT}}{path}" | python3 -c {_shlex.quote(_extract)} '
        "2>/dev/null || true)\"; export SWARM_SMOKE_TOKEN; "
        # 绝不 echo token 本体（敏感值不入冒烟输出/日志）——只报有/无
        'echo "__ACCEPT_LOGIN__:$([ -n "$SWARM_SMOKE_TOKEN" ] && echo ok || echo empty)"'
    )
