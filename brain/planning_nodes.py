...
严格输出 JSON：
{
  ...
  "flow": "业务流程（文字）",
  "file_plan": [{"path": "完整相对路径", "action": "create|modify", "module": "所属业务模块(按需求本身的功能域命名，如 user/order/report/auth，同一功能模块用同一名)", "responsibility": "该文件职责", "depends_on": ["前置文件路径"]}],
  "risks": ["风险1"],
  "acceptance": ["验收标准1"],
...
{"fact_issues": [{"claim","verdict":"false|already_exists|uncertain","detail","suggestion"}],
 ...
 "modules": [{"name":"模块名(按功能域命名，如 user-center)","responsibility":"职责","est_files":12,"depends_on":["前置模块名"]}]}"""
