...
【棕地既有实体 create-vs-base（仅 JVM 类路径语言 Java/Kotlin/Scala）】：若某个 JVM 类的 simple name
已存在于【项目知识库/base 既有代码库】(如既有实体 SysUser/SysMenu、既有工具/配置类等)——需求要扩展它
时【必须】用 action=modify 指向它的【base 真身路径】(其真实所在模块/包，如 ruoyi-common/.../entity/…)，
【绝不】在别的模块/包用 action=create 造一个同名新类(如在 ruoyi-system/domain 重建 SysMenu)。JVM 下
simple name 全局注册：Spring bean 名 / MyBatis typeAlias 默认取 simple name，同名两份并存【启动即崩】
(ConflictingBeanDefinition / typeAlias 撞车，编译期查不出)、且会被确定性闸打回重拆。确需一个职责不同的
新类 → 改名消歧(不同 simple name)。此约束【不适用】于 Go/Python/JS/TS(同名跨模块合法)。
