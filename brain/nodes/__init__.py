...
async def _invoke_llm_abortable(llm, messages, total_timeout: float, fallback_llm=None,
    ...
    # R65D-T6③（round65d 规划期 2 条 TTL 泄漏实锤 09:52/09:57）：C7 在飞预留作用域
    # 此前只接了 worker 侧（executor._run_agent）——brain 规划调用被墙钟/chunk-gap
    # 取消时预留无人结算，挂 30min TTL 才'宁可高估'兜底。本函数是规划侧超时取消的
    # 唯一咽喉：正常返回 settle_leaked=False（残留交 TTL），异常/取消路径立即按中止
    # 语义结算（零虚增窗口）。注：作用域不嵌套还原（brain 节点无外层 worker 作用域）。
    from swarm.models import ledger as _c7_ledger
    _c7_scope = _c7_ledger.begin_inflight_scope()
    try:
        ...
        if _skip_primary:
            ...
            try:
                ...
                _c7_ledger.end_inflight_scope(_c7_scope, settle_leaked=False)
                return _r
        ...
        _c7_ledger.end_inflight_scope(_c7_scope, settle_leaked=True)
        raise
