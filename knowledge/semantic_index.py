...
def make_point_id(
    project_id: str, file_path: str, start_line: object, content: str = "",
) -> str:
    """Qdrant point ID 的【单一来源】，按 (project_id, file_path, start_line) 作键。

    D13（P0-7）：全项目共用单集合 swarm_kb，旧口径 uuid5(file_path:start_line) 不含
    project_id → 项目 A/B 同相对路径同起始行（pom.xml:1、README:1 在多项目部署几乎必然）
    互相 upsert 同一 point ID，payload.project_id 被最后写者替换 → 先写项目的该 chunk
    检索静默消失，且其带 project_id 过滤的 prune/delete 无法回收该点（跨项目覆盖删除）。
    本次把 project_id 纳入 key：ID = uuid5(project_id:file_path:start_line)，各项目
    point 空间彼此隔离、对所有项目对称（不含任何项目名/语言/路径写死）。

    A-P1-19 语义保持：同一项目内，预处理全量(codegraph) 与增量(semantic) 两条写入路径
    对同一逻辑 chunk（同文件同起始行）仍产同一 point ID → 互相 upsert 去重，召回不随
    "最后谁写"漂移。content 参数保留以兼容旧调用签名，不参与 ID 计算。

    fail-closed：project_id 缺失/为空 → 抛 ValueError 拒绝生成 ID。绝不静默退回
    "不含 project_id"的旧口径——那会造成新旧口径混写、跨项目覆盖复发。

    旧口径存量点收敛：改口径后旧 ID 点不再被新写入覆盖，但其 payload
    （project_id / file_path / index_generation / index_source）完整，而本系统全部
    删除/回收路径均按 payload 过滤、不按 ID 反推：
    - 增量路径 reindex_file_atomic → prune_file_stale 按 (project_id, file_path,
      代际 must_not) 删 → 单文件重索引即回收该文件旧 ID 点；
    - 预处理全量路径（project/preprocess.py _store_vectors_qdrant 末尾 prune）按
      (project_id, index_source=codegraph, 代际 must_not) 删 → 项目全量重索引即
      回收本项目全部 codegraph 旧 ID 点；
    - delete_by_file / delete_by_project / reconcile_orphan_points 同为 payload 过滤。
    故存量旧点随现有 reindex/prune 流程确定性收敛，无需一次性清理脚本。
    """
    pid = project_id.strip() if isinstance(project_id, str) else ""
    if not pid:
        logger.warning(
            "make_point_id: project_id 缺失/为空，拒绝生成 point ID（fail-closed，"
            "避免退回无项目隔离的旧口径造成跨项目覆盖）"
        )
        raise ValueError(
            "make_point_id requires a non-empty project_id (D13 fail-closed)"
        )
    key = f"{pid}:{file_path}:{start_line if start_line is not None else 0}"
        ...
        for i in range(0, len(chunks), batch_size):
            ...
            for chunk, vector in zip(batch, vectors):
                point_id = make_point_id(
                    project_id, chunk.file_path, chunk.start_line, chunk.content,
                )
                payload = {
