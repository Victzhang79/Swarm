...
def make_point_id(file_path: str, start_line: object, content: str) -> str:
    """P1-DEBT-04：Qdrant point ID 的【单一来源】。

    预处理全量(codegraph) 与增量(semantic) 此前用两套不相交方案(blake2b int vs uuid5 str)
    写同一集合 → 同一逻辑 chunk 生成不同 ID、无法 upsert 去重 → 召回随"最后谁写"漂移。
    统一为 uuid5(file_path:start_line:content[:64])，两条路径对同一 (文件,行,内容) 产同一 ID。
    """
    key = f"{file_path}:{start_line if start_line is not None else 0}:{(content or '')[:64]}"
